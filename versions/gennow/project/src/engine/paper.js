...
(function (G) {
  ...
  // ---- fonts / styles -------------------------------------------------------------
  // gen now version: the brand fonts from gennow.de – Bricolage Grotesque for
  // headlines and numbers, Lato for text – plus Caveat for handwriting.
  // (The keys keep their original names so timeline.js stays compatible.)
  const BRIC = '"Bricolage Grotesque"';
  E.FONTS = {
    headline: { family: BRIC, weight: 800, style: 'normal', size: 84, lh: 1.04 },
    headlineItalic: { family: BRIC, weight: 500, style: 'normal', size: 74, lh: 1.08 }, // lighter "tagline" cut, as on gennow.de
    serifSmall: { family: BRIC, weight: 700, style: 'normal', size: 48, lh: 1.12 },
    shout: { family: BRIC, weight: 800, style: 'normal', size: 72, lh: 1.04, upper: true, ls: 0.5 },
    number: { family: BRIC, weight: 800, style: 'normal', size: 150, lh: 1.0 },
    body: { family: 'Lato', weight: 700, style: 'normal', size: 40, lh: 1.22 },
    small: { family: 'Lato', weight: 700, style: 'normal', size: 32, lh: 1.25 },
    kicker: { family: 'Lato', weight: 900, style: 'normal', size: 30, lh: 1.2, upper: true, ls: 2.5 },
    typed: { family: 'Lato', weight: 700, style: 'normal', size: 36, lh: 1.22 },
    hand: { family: 'Caveat', weight: 700, style: 'normal', size: 58, lh: 1.05 },
    caption: { family: 'Lato', weight: 700, style: 'normal', size: 42, lh: 1.25 },
  };
  E.fontStr = function (f, size) {
    return `${f.style} ${f.weight} ${size || f.size}px ${f.family}`;
  };

  // gen now palette (sampled from gennow.de): beige #F3E8DC, blue #0C55A6,
  // sky #1EB4FF, green #7EEAAC, pink #F782C3, lilac #CDB7EC, purple #8C47ED,
  // brush-stroke yellow #FCDF48, ink #141414.
  const INK = '#141414';
  E.COLORS = {
    white: { bg: '#FFFFFF', fg: INK },
    cream: { bg: '#FFF8EE', fg: INK },
    ink: { bg: '#141414', fg: '#FFFFFF' },
    blue: { bg: '#0C55A6', fg: '#FFFFFF' },
    sky: { bg: '#1EB4FF', fg: INK },
    yellow: { bg: '#FCDF48', fg: INK },
    yellowLite: { bg: '#FEEE9E', fg: INK },
    pink: { bg: '#F782C3', fg: INK },
    green: { bg: '#7EEAAC', fg: INK },
    lilac: { bg: '#CDB7EC', fg: INK },
    purple: { bg: '#8C47ED', fg: '#FFFFFF' },
    lined: { bg: '#FFFFFF', fg: '#0C55A6' },
  };
  // names from the original version, mapped onto the brand palette
  Object.assign(E.COLORS, {
    red: E.COLORS.blue, mustard: E.COLORS.yellow, lemon: E.COLORS.yellowLite,
    mint: E.COLORS.green, charcoal: E.COLORS.ink, teal: E.COLORS.purple,
  });
  ...
  E.makeStamp = function (text, color = '#2E7D4F', size = 64, seed = 's') {
    return E.cached('stamp|' + text + color + size + seed, () => {
      const tmp = E.canvas(4, 4).getContext('2d');
      tmp.font = `800 ${size}px "Bricolage Grotesque"`;
  ...
  // ---- backgrounds -------------------------------------------------------------------
  // scene sheets in the gen now colours (the site's section colours)
  E.BG = {
    beige: '#F3E8DC',
    sky: '#1EB4FF',
    yellow: '#FCDF48',
    ...
    lilac: '#CDB7EC',
    blue: '#0C55A6',
    charcoal: '#0C55A6', // the opening sheet is brand blue in this version
  };
  const DARK_BG = { blue: 1, charcoal: 1 };
  E.bgOverscan = 60;
  E.makeBackground = function (name, W, H) {
    return E.cached('bg|' + name + W + 'x' + H, () => {
      ...
      for (let i = 0; i < 38; i++) {
        ...
        const g = ctx.createRadialGradient(x, y, 0, x, y, rad);
        const light = r() < 0.5;
        g.addColorStop(0, light ? `rgba(255,250,235,${dark ? 0.03 : 0.07})` : `rgba(60,40,20,${dark ? 0.08 : 0.045})`);
        g.addColorStop(1, 'rgba(0,0,0,0)');
      ...
      ctx.save();
      ctx.globalCompositeOperation = 'soft-light';
      ctx.globalAlpha = dark ? 0.45 : 0.5; // lighter grain than the original, so the brand colours stay true
      for (let y = 0; y < h; y += 512) for (let x = 0; x < w; x += 512) ctx.drawImage(gr, x, y);
      ctx.restore();
      // gen now: soft-light grain darkens light colours on average; add back the
      // measured shift so each sheet averages to its exact brand colour
      const probe = ctx.getImageData(Math.round(w * 0.2), Math.round(h * 0.2), Math.round(w * 0.6), Math.round(h * 0.6)).data;
      const sum = [0, 0, 0];
      for (let i = 0; i < probe.length; i += 16) { sum[0] += probe[i]; sum[1] += probe[i + 1]; sum[2] += probe[i + 2]; }
      const n = probe.length / 16, hex = base.replace('#', '');
      const want = [0, 2, 4].map((k) => parseInt(hex.substr(k, 2), 16));
      const add = want.map((v, k) => Math.max(0, Math.round(v - sum[k] / n)));
      ctx.save();
      ctx.globalCompositeOperation = 'lighter';
      ctx.fillStyle = `rgb(${add[0]},${add[1]},${add[2]})`;
      ctx.fillRect(0, 0, w, h);
      ctx.restore();
      // vignette
      const vg = ctx.createRadialGradient(w / 2, h * 0.45, h * 0.18, w / 2, h / 2, h * 0.72);
      vg.addColorStop(0, 'rgba(0,0,0,0)');
      vg.addColorStop(1, dark ? 'rgba(0,10,40,0.3)' : 'rgba(40,25,10,0.09)');
