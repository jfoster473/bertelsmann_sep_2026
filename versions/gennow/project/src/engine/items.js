...
(function (G) {
  ...
  E.itemSprite = function (it) {
    if (it.type === 'strip') {
      return E.makeStrip({ text: it.text, font: it.font, size: fitSize(it), color: it.color, seed: it.id, align: it.align, rim: it.rim, ink: it.ink });
    }
    if (it.type === 'caption') {
      return E.makeStrip({ text: it.text, font: 'caption', color: 'ink', seed: it.id, pad: [34, 18], rim: '#5E5E66', amp: 1.6, endAmp: 3.5 });
    }
    if (it.type === 'stamp') return E.makeStamp(it.text, it.ink, it.size, it.id);
    if (it.type === 'logocard') return E.makeLogoCard(it); // gen now version (engine/brand.js)
    return null;
  };
