...
(function (G) {
  ...
  E.render = function (ctx, t) {
    ...
    // opening: the brand-blue gen now sheet tears away upwards
    if (tr.type === 'tearOpen' && inTrans) {
      const p = 1 - E.easeInCubic(0.2 + 0.8 * ((t - sc.start) / tr.dur));
      E.drawSheet(ctx, E.makeOpenSheet(W, H), p, 'top', 'open', W, H, cam);
    }
    ...
    // gen now corner logo: fixed to the frame, not moved by the camera
    E.drawWatermark(ctx, t, TL);
  };
  ...
  E.prewarm = function () {
    ...
    E.makeOpenSheet(TL.width, TL.height);
