...
(function (G) {
  ...
  E.icon = function (name, size = 70) {
    return E.makeCutout('icon-' + name + size, size, size, (ctx) => {
      ...
      ctx.scale(k, k);
      if (name === 'lock') {
        ctx.strokeStyle = '#3A3633'; ctx.lineWidth = 9;
        ctx.beginPath(); ctx.arc(0, -8, 16, Math.PI, 0); ctx.lineTo(16, 4); ctx.moveTo(-16, -8); ctx.lineTo(-16, 4); ctx.stroke();
        ctx.fillStyle = '#C8483C'; ctx.beginPath(); ctx.roundRect(-26, -2, 52, 38, 7); ctx.fill();
        ctx.fillStyle = '#2E2B28'; ctx.beginPath(); ctx.arc(0, 13, 6, 0, 7); ctx.fill(); ctx.fillRect(-3, 14, 6, 12);
      } else if (name === 'question') {
        ctx.fillStyle = '#3F7FA8'; ctx.beginPath(); ctx.arc(0, 0, 32, 0, 7); ctx.fill();
        ctx.fillStyle = '#F4EEE2'; ctx.font = '900 46px "Playfair Display"'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillText('?', 0, 3);
      } else if (name === 'bolt') {
        ...
        ctx.fillStyle = '#F2DC7A'; ctx.beginPath(); ctx.arc(0, -8, 24, 0, 7); ctx.fill();
        ctx.fillStyle = '#8F8A80'; ctx.fillRect(-12, 14, 24, 16);
      } else if (name === 'heart') {
        ctx.fillStyle = '#C8483C'; ctx.beginPath(); ctx.moveTo(0, 26); ctx.bezierCurveTo(-40, 0, -24, -34, 0, -12); ctx.bezierCurveTo(24, -34, 40, 0, 0, 26); ctx.fill();
      }
  ...
  function surveyCard() {
    ...
    return E.makeCard({ w: c.w, h: c.h, color: '#F8F5EE', seed: 'survey', lined: true, draw: (ctx, w, h) => {
      ctx.fillStyle = '#1E1B18'; ctx.font = '700 38px "Courier Prime"'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
      ctx.fillText(c.title, -w / 2 + 34, -h / 2 + 44);
      ctx.fillStyle = '#C8483C'; ctx.fillRect(-w / 2 + 34, -h / 2 + 70, 120, 5);
      for (let i = 0; i < 3; i++) {
        const y = -h / 2 + 130 + i * 88;
        ctx.strokeStyle = '#2B2927'; ctx.lineWidth = 4; ctx.strokeRect(-w / 2 + 38, y - 18, 36, 36);
  ...
  function drawCard(ctx, t) {
    ...
    c.ticks.forEach((tt, i) => {
      ...
      ctx.strokeStyle = '#2B55A8'; ctx.lineWidth = 7; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
      ctx.beginPath();
  ...
  S.barriers = {
    ...
    drawTop(ctx, t) {
      ...
      if (k && t >= k.at && t < 33.2) {
        ...
        ctx.save();
        ctx.strokeStyle = 'rgba(184,52,42,0.92)'; ctx.lineWidth = 9; ctx.lineCap = 'round';
        ctx.beginPath();
  ...
  S.gem = {
    draw(ctx, t) {
      ...
      g.bars.forEach((bar, i) => {
        ...
        const nd = d - 0.55;
        if (nd > 0) {
          const nv = E.makeStrip({ text: bar.text, font: 'number', size: i ? 80 : 62, color: i ? 'red' : 'white', seed: 'gemv' + i });
      ...
      if (ap > 0) {
        ...
        E.drawHandArrow(ctx, x1 - 30, y1, x2 - g.barW / 2 - 30, y2 + 30, ap, '#2A1C18', 7, -0.3);
      }
    },
  };

  // ============================================================== CONFETTI ===
  const CONF_COLORS = ['#E3B64E', '#F1EBDF', '#C8483C', '#9CC0E0', '#2F6F69', '#EDBDB4', '#D4B265'];
